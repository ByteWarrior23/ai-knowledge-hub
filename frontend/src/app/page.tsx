      ...
      <div className="space-y-4 text-center max-w-md">
        {steps.map((step, i) => (
          <motion.div
            ...
            className={`text-sm tracking-wide font-mono ${i === steps.length - 1 ? 'text-primary' : 'text-muted-foreground'}`}
          >
            {i === steps.length - 1 ? (
    ...
    >
      ...
      >
        <div className="absolute inset-0 backface-hidden">
          <SpotlightCard className="h-full flex flex-col items-center justify-center p-8 text-center hover:border-primary/40 transition-colors shadow-2xl shadow-black/20">
            <Badge variant="secondary" className="mb-8 text-muted-foreground border-0">Card {idx + 1}</Badge>
            <h3 className="font-medium text-xl text-foreground leading-snug">{card.question}</h3>
            <div className="absolute bottom-8 flex items-center gap-2 text-[10px] font-mono uppercase tracking-widest text-muted-foreground group-hover:text-primary transition-colors">
              <span>Reveal Answer</span> <ArrowRight size={10} />
            </div>
          </SpotlightCard>
        </div>
        <Card className="absolute inset-0 backface-hidden rotate-y-180 flex flex-col items-center justify-between p-8 text-center bg-card border-border backdrop-blur-md">
            <div className="flex-grow flex items-center justify-center overflow-y-auto w-full">
              <p className="text-foreground text-lg leading-relaxed font-medium">{card.answer}</p>
            </div>
            <div className="flex gap-3 w-full mt-4">
                <Button 
                  variant="outline" 
                  className="flex-1 rounded-lg border-border text-muted-foreground hover:text-foreground" 
                  onClick={(e) => { 
                    e.stopPropagation(); 
                ...
                <Button 
                  className="flex-1 rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground border-0" 
                  onClick={(e) => { 
                    e.stopPropagation(); 
      ...
      <footer className="w-full py-20 mt-32 border-t border-border bg-background">
         <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row justify-between items-end gap-12">
            ...
            <div className="flex flex-col items-end gap-6 text-right">
              ...
              <p className="text-muted-foreground text-sm">AI Knowledge Hub - Document Intelligence with RAG.</p>
              <div className="flex gap-6 mt-2">
                <a href="https://github.com/ByteWarrior23/ai-knowledge-hub" className="text-muted-foreground hover:text-foreground transition-colors text-sm">GitHub</a>
                <a href="https://github.com/ByteWarrior23" className="text-muted-foreground hover:text-foreground transition-colors text-sm">Portfolio</a>
              </div>
            </div>
         </div>
      </footer>
